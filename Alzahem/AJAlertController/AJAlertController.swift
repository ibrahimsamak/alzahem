...
class AJAlertController: UIViewController {
    ...
    private func show()
    {
        if let appDelegate = UIApplication.shared.delegate, let window = appDelegate.window, let rootViewController = window?.rootViewController {
            ...
            UIView.animate(withDuration: 0.15 , delay: 0.1, options: .curveEaseOut, animations: { () -> Void in
    ...
    private func hide()
    {
        ...
        self.viewAlert.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
        UIView.animate(withDuration: 0.20, delay: 0.0, options: .curveEaseIn, animations: { () -> Void in
        ...
        UIView.animate(withDuration: 0.20, delay: 0.05, options: .curveEaseIn, animations: { () -> Void in
            self.view.alpha = 0.0
